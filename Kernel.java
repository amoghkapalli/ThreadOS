...
public class Kernel
{
	...
	case INTERRUPT_SOFTWARE: // System calls
        ...
        if ((myTcb = scheduler.getMyTcb()) != null) {
            FileTableEntry ret =  myTcb.getFtEnt(param);
            filesystem.close((int)(args));
            if (myTcb.returnFd(param) != ret){
                return ERROR;
            }
            else {
                return ERROR;
	    ...
	    case SIZE:
        if ((myTcb = scheduler.getMyTcb()) != null) {
            int size = filesystem.fsize((int)args);
            return size;
        }
        else {
            return ERROR;
        ...
        if ((myTcb = scheduler.getMyTcb()) != null) {
            ...
            int seekptr = filesystem.seek(ftEnt, i[0], i[1]);
            return seekptr;
        }
        else {
            return ERROR;
	    ...
	    case FORMAT:
		//filesystem=new FileSystem(disk.blockSize);
        if ((myTcb = scheduler.getMyTcb()) != null) {
            int ret = filesystem.format((int)args);
            if (ret == 0) {
                return OK;
            }
            else {
